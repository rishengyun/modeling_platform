...
如果当前平台已经部署完成，而你只是新增平台功能，不需要重新初始化整套环境。通常只需要处理下面两类事情：

1. **数据库增量升级**
   - 新增的表结构位于：`postgres/init/07-platform_upgrade.sql`
   - 由于 PostgreSQL 的 `docker-entrypoint-initdb.d` 只会在数据库第一次初始化时自动执行，因此已有数据库不会自动再次跑这个脚本。
   - 需要手动执行：

```bash
psql -h <postgres_host> -U <postgres_user> -d postgres -f /项目路径/postgres/init/07-platform_upgrade.sql
```

如果 PostgreSQL 是 Docker 容器，也可以执行：

```bash
docker cp /项目路径/postgres/init/07-platform_upgrade.sql mypostgres:/tmp/07-platform_upgrade.sql
docker exec -it mypostgres psql -U postgres -d postgres -f /tmp/07-platform_upgrade.sql
```

2. **服务增量重部署**
   - 本次新增的平台能力主要位于 `task-service`，因此至少需要重新部署 `task-service`
   - 如果你访问的是服务器上已经部署好的前端页面，还需要重新打包前端并替换 `dist`
   - 一般不需要重部署 `gateway`、`fastapi-main-api`、`graph-service`
   - 如果要启用真实大模型能力，还需要在 `task-service` 的运行环境中配置 `DEEPSEEK_API_KEY`

如果新增页面打开后接口返回 `404`，最常见原因就是服务器上的 `task-service` 仍然是旧版本。

#### 2.2.5 平台真实 LLM 配置

当前平台的工作流生成、数据把脉、业务报告、图谱解释、Agent 算子已经支持接入真实 LLM。默认接入方式为：

- Provider: `DeepSeek`
- Model: `deepseek-v4-pro`

部署时请为 `task-service` 注入以下环境变量：

```bash
PLATFORM_LLM_ENABLED=true
PLATFORM_LLM_PROVIDER=deepseek
PLATFORM_LLM_BASE_URL=https://api.deepseek.com
PLATFORM_LLM_MODEL=deepseek-v4-pro
PLATFORM_LLM_TEMPERATURE=0.2
PLATFORM_LLM_MAX_TOKENS=3200
...
1. 建议只在服务器环境文件中配置真实密钥，不要将密钥写入仓库。
2. 如果未配置 `DEEPSEEK_API_KEY`，平台会自动退回到本地规则结果，不会导致页面整体不可用。
...
/data/HuangXX/modeling_platform/model-backend/modeling-platform-fastapi-backend
```
